...
// region Add
#include <winrt/Windows.Devices.Power.h> // 用于访问电池API
// end region 

using namespace winrt::Windows::UI::Xaml;
namespace Power = winrt::Windows::Devices::Power;
...
// region add
FormattedString<FORMATTED_BUFFER_SIZE> g_batteryRateFormatted;
// end region
...
// *** 新增函数：获取电池实时功耗 (WinRT 版本) ***
// Returns a struct containing Rate (in mW) and PowerState.
// Rate can be positive (charging) or negative (discharging).
std::optional<BatteryInfo> GetBatteryPowerRate() {
    try {
        // 1. 获取所有电池的聚合报告
        auto aggregateBattery = Power::Battery::AggregateBattery();
        auto report = aggregateBattery.GetReport();
        ...
        // - 放电时，值为负数
        // - 充满时，值为 null 或 0
        // 这与我们的 BatteryInfo 结构设计非常契合，但要注意正负号的约定
        auto chargeRateOpt = report.ChargeRateInMilliwatts();
        if (!chargeRateOpt) {
            // 如果功耗值无效 (null)，说明无法获取，可能是AC供电且充满
            Wh_Log(L"Battery: ChargeRateInMilliwatts is not available. Possibly fully charged on AC.");
            // 我们可以返回一个充电/放电率为0的状态
            return BatteryInfo{0, BATTERY_POWER_ON_LINE};
        }

        long chargeRate = chargeRateOpt.Value();
        Wh_Log(L"Battery Status OK (WinRT): Rate=%ld mW", chargeRate);

        // 3. 构建并返回 BatteryInfo 结构体
        // 我们需要把 WinRT 的状态映射回旧的 PowerState 定义
        // 以便格式化函数 GetBatteryRateFormatted 能继续工作
        ULONG powerState = 0;
        if (chargeRate > 0) {
            ...
            powerState = BATTERY_POWER_ON_LINE; // 假设0代表已连接电源且稳定
        }

        // 让我们看看格式化函数：
        // double rateWatts = static_cast<double>(abs(batteryInfo->Rate)) / 1000.0;
        // if (batteryInfo->PowerState & BATTERY_DISCHARGING) { swprintf_s(..., "%.1f W", rateWatts); }
        // else if (batteryInfo->PowerState & BATTERY_CHARGING) { swprintf_s(..., "+%.1f W", rateWatts); }        
        // 因此，我们只需提供一个`Rate`（其绝对值正确）和正确的`PowerState`。
        // 我们直接用 chargeRate（绝对值正确），然后根据其正负设置 PowerState。

        return BatteryInfo{chargeRate, powerState};

    } catch (const winrt::hresult_error& ex) {
        // 捕获可能发生的 WinRT 异常，例如在没有电池的台式机上。
        // hresult_error for "Element not found" indicates no battery.
        if (ex.code() == HRESULT_FROM_WIN32(ERROR_NOT_FOUND)) {
             Wh_Log(L"Battery: No battery found (WinRT exception).");
        } else {
             Wh_Log(L"Battery: WinRT exception while getting report: %ls (0x%08X)", ex.message().c_str(), ex.code());
        }
        return std::nullopt;
    } catch (...) {
        Wh_Log(L"Battery: An unknown error occurred while using WinRT Power API.");
        return std::nullopt;
    }
}
...
void DataCollectionSessionInit() {
    ...
    // region add
    // 新增，检查是否使用了新模式
    metrics[static_cast<int>(MetricType::kBatteryRate)] =
        IsStrInDateTimePatternSettings(L"%battery_rate%");
    // end region
...
PCWSTR GetBatteryRateFormatted() {
    DWORD dataCollectionFormatIndex = GetDataCollectionFormatIndex();
    if (g_batteryRateFormatted.formatIndex != dataCollectionFormatIndex) {
        auto batteryInfo = GetBatteryPowerRate();
        wchar_t temp[16]; // 临时缓冲区，用于生成核心字符串

        if (batteryInfo.has_value()) {
            long rateMilliwatts = batteryInfo->Rate;
            double rateWatts = static_cast<double>(abs(rateMilliwatts)) / 1000.0;

            if (rateMilliwatts == 0) {
                // 核心字符串: "0.0W"
                wcscpy_s(temp, L"0.0W");
            } 
            else if (rateMilliwatts > 0) { // 充电 (+)
                if (rateWatts < 10.0) {
                    // 核心字符串: "+8.7W" (5字符) - 空间足够显示小数
                    swprintf_s(temp, L"+%.1fW", rateWatts);
                } else {
                    // 核心字符串: "+75W" 或 "+105W" - 空间不足，舍弃小数
                    swprintf_s(temp, L"+%.0fW", round(rateWatts));
                }
            } 
            else { // 放电 (无符号)
                if (rateWatts < 100.0) {
                    // 核心字符串: "8.7W" 或 "75.2W" - 空间足够显示小数
                    swprintf_s(temp, L"%.1fW", rateWatts);
                } else {
                    // 核心字符串: "105W" - 空间不足，舍弃小数
                    swprintf_s(temp, L"%.0fW", round(rateWatts));
                }
            }
        } else {
        ...
        // --- 最终格式化步骤 ---
        // 将 temp 缓冲区中的核心字符串右对齐地写入 g_batteryRateFormatted.buffer
        // %5s 会自动在左侧填充空格，以达到5个字符的总宽度。
        swprintf_s(g_batteryRateFormatted.buffer, FORMATTED_BUFFER_SIZE, L"%5s", temp);
        
        g_batteryRateFormatted.formatIndex = dataCollectionFormatIndex;
    }
    return g_batteryRateFormatted.buffer;
}
...
size_t ResolveFormatToken(
    ...
    struct {
        ...
        // *** 新增这一行 ***
        {L"%battery_rate%"sv, GetBatteryRateFormatted},
